...
void postAddTapped(Post postToAdd){
  print(postsNotifier.value.length);
   ...
   */
  MyListViewNotifier<Post> postsNotifier = MyListViewNotifier<Post>([
    Post(
      userId: 1,
      id: 1,
      title: "sunt aut facere repellat provident occaecati excepturi optio reprehenderit",
...
class MyListViewNotifier<T> extends ValueNotifier<List<T>>{
  ...
  void postRemoveTapped(T postToRemove){
    value.remove(postToRemove);
    notifyListeners();
  }
  void postAddTapped(T postToAdd){
    value.add(postToAdd);
    notifyListeners();
